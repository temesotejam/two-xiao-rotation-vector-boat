...
bool Pca9685::setFullOff(uint8_t ch) { if (ch > 15) return false; const uint8_t d[4] = {0, 0, 0, 0x10}; if (!writeN(LED0_ON_L + 4 * ch, d, 4)) return false; PcaChannel r{}; if (!readChannel(ch, r) || !r.fullOff) { ++readbackErrors; return false; } return true; }
bool Pca9685::allOff() { for (uint8_t ch = 0; ch < 16; ++ch) if (!setFullOff(ch)) return false; return true; }
...
bool Pca9685::setPulse(uint8_t ch, uint16_t us, PcaChannel* actual) { if (ch > 15) return false; if (us < 500) us = 500; if (us > 2500) us = 2500; uint32_t count = ((uint32_t)us * 4096UL * (uint32_t)(kServoPwmHz + 0.5f) + 500000UL) / 1000000UL; if (count > 4095) count = 4095; const uint8_t d[4] = {0, 0, (uint8_t)(count & 0xff), (uint8_t)((count >> 8) & 0x0f)}; if (!writeN(LED0_ON_L + 4 * ch, d, 4)) return false; PcaChannel r{}; if (!readChannel(ch, r)) { ++readbackErrors; return false; } if (r.on != 0 || r.off != count || r.fullOff) { ++readbackErrors; return false; } if (actual) *actual = r; return true; }
bool Pca9685::readChannel(uint8_t ch, PcaChannel& o) { if (ch > 15) return false; const uint8_t r = LED0_ON_L + 4 * ch; Wire.beginTransmission(kPcaAddress); Wire.write(r); if (Wire.endTransmission(false) != 0 || Wire.requestFrom((int)kPcaAddress, 4) != 4) { ++errors; return false; } const uint8_t a = Wire.read(), b = Wire.read(), c = Wire.read(), d = Wire.read(); o.on = ((uint16_t)(b & 0x0f) << 8) | a; o.off = ((uint16_t)(d & 0x0f) << 8) | c; o.fullOff = (d & 0x10) != 0; return true; }
